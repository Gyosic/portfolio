...
import { stamp as auditStamp } from "@/lib/audit";
...
export const config = {
  matcher: [
    `/((?!favicon.ico|api/rooms|api/users|_next/image|_next/static|workers/|welcome/|avatars/|[^/]+/signin|[^/]+/signup|[^/]+/error|[^/]+/tv|[^/]+/meet|[^/]+/languages/.+|api/auth/.+).{1,})`,
    `/`,
  ],
};

const getSigninPathname = (req: NextAuthRequest, language: Language) => {
  const { origin, pathname: callbackUrl } = req.nextUrl;
  return `${origin}/${language}${authConfig.pages.signIn}?${new URLSearchParams({ callbackUrl })}`;
};
...
const redirectToSignin = (req: NextAuthRequest, language: Language) => {
  const url = new URL(getSigninPathname(req, language));
...
const PostMiddleware = (res: NextResponse) => {
  // 버전정보 삽입
  res.headers.set("X-AMI-Version", version);
...
const middleware = auth(async (req: NextAuthRequest) => {
  ...
  const language: Language =
    (req.auth?.user.properties.language?.split("_")?.[0] as Language) || getLocale(req);
  ...
  // 세션정보가 담겨있다면 바로 통과
  if (Boolean(req.auth)) return PostMiddleware(NextResponse.next());
