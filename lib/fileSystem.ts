...
class FileSystem {
  ...
  async init() {
    FileSystem._storageRoot = process.env.FILE_STORAGE_ROOT || "/tmp/portfolio";
  ...
  destination() {
    ...
    const destination = path.join(
      this.storageAbsolutePathname(),
      [now.getFullYear(), now.getMonth() + 1, now.getDate(), now.getHours()].join("/"),
    );
