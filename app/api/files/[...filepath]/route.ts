...
const tableMap: Record<string, PgTable & { filename: Column }> = {
  firmwares,
};
...
export async function GET(req: NextRequest, { params }: { params: Promise<Params> }) {
  try {
    ...
    const [tableName, filename] = _filepath.slice(-2);
    const table = tableMap[tableName];

    const headers = new Headers();

    if (table) {
      const rows = await db.select().from(table).where(eq(table.filename, filename));
      const [{ originalname }] = rows;
      if (originalname)
        headers.set(
          "Content-Disposition",
          `attachment; filename="${encodeURIComponent(originalname as string)}"`,
        );
    }
